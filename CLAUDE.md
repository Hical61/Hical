...
- `HttpServer.h` — Top-level facade integrating TcpServer + Router + MiddlewarePipeline + WebSocket middleware pre-build + fd exhaustion handling. SO_REUSEPORT multi-acceptor model (Linux/macOS) with single-acceptor fallback (Windows). Per-io_context `IdleScanner` instances for centralized idle connection scanning (replaces per-connection timer coroutines)
