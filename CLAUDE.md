...
- `HttpResponse.h/cpp` — HTTP response wrapper. `NativeResponse` stores owned `HeaderMap` + `string body` + `optional<FileBody> fileBody` (path/offset/length for deferred async file sending). `serialize()` produces full HTTP wire bytes. `serializeHeadTo(FixedBuffer<512>&)` for zero-heap-alloc scatter-gather I/O. `serializeHeadTo(buf, prefix, prefixLen)` overload accepts pre-built common headers (Server/Connection/Date wire bytes) to skip per-request insert+serialize. `preparePayload()` sets Content-Length via `std::to_chars` (supports both string body and FileBody). `HttpResponse` class provides `setFileBody()`, `hasFileBody()`, `rangeNotSatisfiable()` factory. `setHeader()` accepts `std::string_view` — literal arguments avoid temporary `std::string` construction
- `HttpSessionImpl.cpp` — Compilation firewall isolating picohttpparser + self-developed WebSocket implementation. **ReadBufferPool borrow/return**: per-request `ReadBufferPool::acquire()` borrows an 8KB `std::string` from thread_local pool; returned after response is written; idle connections hold zero read buffer. Pipeline spill stored in `pipelineSpill` (`std::string` outside `for(;;)`, empty for most connections — SSO zero heap). **Optimistic sync write**: `tryOptimisticWrite()` attempts `write_some()` before `async_write`; small responses (head+body ≤512B merged into `FixedBuffer<512>`) complete synchronously without suspending the coroutine or queuing into the reactor completion queue — `would_block`/partial/hard-error fall back to `async_write`. `handleSession` entry explicitly calls `socket.non_blocking(true)` to guarantee `write_some` never enters blocking `poll()` on EAGAIN (Asio's internal `O_NONBLOCK` flag doesn't count — `sync_send` only checks `user_set_non_blocking`). **Early readBuf release**: `readBufHandle.release()` immediately after response write + pipeline spill save + memmove, instead of waiting for coroutine frame destruction — cuts peak in-flight buffer memory by ~50% under high concurrency. **Response prefix template**: connection-level `responsePrefix[128]` pre-builds `Server`/`Connection`/`Date` wire bytes (~90B), keep-alive requests use `memcpy` instead of 3× `HeaderMap::insert` + serialize loop; Date updated once per second (29B memcpy). Single-buffer serialization: head+body into one `FixedBuffer<512>`, one `async_write` call. `writeFileResponse()` coroutine for `FileBody` responses (Range requests): `TcpCorkGuard` RAII (Linux `TCP_CORK` / macOS `TCP_NOPUSH` / Windows no-op) merges header + first 64KB chunk into one TCP segment; sends headers first, then 64KB async chunked file reads. **Chunked body send path**: `writeResponse()` / `writeResponseWithPrefix()` identify chunked body, send head first then batch all chunk frames. **SSE session handling**: `handleSseSession()` routes GET requests to SSE handler with IdleScanner registration (30 min timeout), 30s heartbeat coroutine, and user onConnect callback. WebSocket bridge: reconstructs native request at upgrade time (rare path). Idle timeout via `IdleScanner::Guard` RAII registration: `Entry` on coroutine stack (zero heap allocation), `touch()` on request read/write; scanner is nullptr when `idleTimeout_ == 0` (Guard becomes no-op). `SocketGuard` conditional shutdown: only calls `shutdown()` on clean keep-alive exit, eliminating useless syscalls on already-disconnected sockets
- `Router.h` — Static routes (hash map O(1) with transparent hashing via `RouteKeyView`/`is_transparent` for zero-alloc `string_view` lookup) + parameter routes (`{id}` pattern, per-method grouping via `unordered_map<HttpMethod, vector>`) + wildcard routes (`*path`, priority: static > param > wildcard, captures remaining path segments) + WebSocket routes with `WsOptions` (Origin whitelist) + SSE routes (`sse()` callback registration). `dispatchSync()` synchronous fast path: sync-registered handlers skip coroutine frame allocation (~40-130ns/req). `resolveRoute()` unifies URL decode + path depth check + static/param/wildcard lookup for both `dispatch()` and `dispatchSync()`
- `Middleware.h` — Onion-model middleware pipeline with `MiddlewareNext` chaining; supports pre-built chain (`build()`), dynamic chain (`buildChain()`), and `buildFor()` for external pre-build, with separate `execute()` overloads for cached vs dynamic paths. `SyncBeforeHandler` / `SyncAfterHandler` / `SyncMiddlewareResult` types for zero-coroutine-frame middleware. `MiddlewareEntry` tagged union (Async/Sync). `buildOptimizedChain()`: consecutive sync entries merged into single coroutine frame, N sync middleware = 1 heap allocation. **Forwarding coroutine elimination**: Async wrapper lambdas return `mw(r, next)` directly instead of `co_return co_await mw(r, next)` — removes one coroutine frame per async middleware layer (mw10 = 10 frames saved per request)
...
- `StringPool.h` — thread_local size-class string object pool (5 tiers: 256/512/1K/2K/4K bytes, 32 slots each). `acquire(size)` returns a `shared_ptr<string>` pre-reserved to the matching tier with a custom deleter that returns it to the pool on release. `send(const char*, size_t)` and `send(shared_ptr<PmrBuffer>)` use pool paths to avoid per-send malloc/free. `send(string&&)` bypasses the pool (move is cheaper than acquire+assign). Cross-thread release degrades to normal new/delete (main scenario: IO thread send → writeLoop same thread, pool path is effective). Max resident memory: 32 × 5 × 4096 = 640KB per thread
...
- `EventLoopPool` — Multi-threaded pool (1 thread : 1 io_context), least-connections distribution (picks the loop with the smallest `connectionCount_`). `AsioEventLoop` exposes `atomic<size_t> connectionCount_`; `TcpServer` increments/decrements it in `addConnection()`/`removeConnection()`. `start()` binds each worker thread to its corresponding CPU core via `pthread_setaffinity_np` (Linux only) to reduce TLB flush and cross-core IPI from thread migration
...
- `DbConnectionPool.h/cpp` — Coroutine-based connection pool using `steady_timer` as coroutine semaphore (no `condition_variable`). LIFO idle connection reuse, background `healthCheckLoop` (ping + replenish to `minConnections`), `idleCheckLoop` (evict idle beyond `idleTimeout` — precise wake-up: `min_element` finds the oldest idle connection, timer fires exactly at its expiry time; `idleCheckInterval` as max-wait fallback), `acquire()` pre-checks `isAlive()` inside the lock to fast-evict dead connections (dead connections collected then destructed outside the lock), `pingGracePeriod` optimization (skip ping if recently checked), automatic rollback on release if `inTransaction()`. Factory pattern via `DbConnectionFactory` function type
...
- **Zero-copy HTTP parsing**: picohttpparser parses into stack-allocated `phr_header[64]` array. `NativeRequest` stores `string_view` referencing per-request `readBuf` (borrowed from `ReadBufferPool`, returned after response is written) — zero heap allocation for headers/target. Body uses owned `std::string` (read directly from socket). Single-buffer response serialization: head+body into `FixedBuffer<512>`, one `async_write` call. Idle connections hold no heap read buffer (256B stack buffer for speculative `async_read_some`, eliminating `async_wait` → `epoll_ctl(MOD)` overhead).
...
- `test_query_params.cpp` / `test_form_params.cpp` — Query parameter and form parameter parsing
...
- `test_graceful_shutdown.cpp` — Graceful shutdown with signal handling
- `test_error_handler.cpp` — Global error handler
- `test_expect_continue.cpp` — Expect: 100-continue support
- `test_http_client.cpp` — HTTP client test utility (chainable API)
