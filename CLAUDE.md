...
- **Zero-copy HTTP parsing**: picohttpparser parses into stack-allocated `phr_header[64]` array. `NativeRequest` stores `string_view` referencing per-request `readBuf` (borrowed from `ReadBufferPool`, returned after response is written) — zero heap allocation for headers/target. Body uses owned `std::string` (read directly from socket). Single-buffer response serialization: head+body into `FixedBuffer<512>`, one `async_write` call. Idle connections hold no read buffer (v2.6.5).
