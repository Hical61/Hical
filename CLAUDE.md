...
- `RateLimiter.h/cpp` — Token Bucket 速率限制中间件：按 key（默认 IP）独立限流，per-bucket mutex + shared_mutex map 双重检查，burst 瞬时爆发支持，429 + Retry-After/X-RateLimit-* 响应头，maxEntries 防 DoS，惰性 GC 定期清理过期桶。`makeRateLimiterMiddleware()` 返回 `SyncBeforeHandler`（零协程帧开销）
- `ChunkedBody.h/cpp` — Chunked Transfer-Encoding 响应体封装（RFC 7230 section 4.1），提供收集模式（多次 write 后 end）和流式 chunk 编码工具函数（`serializeChunkFrame` / `serializeTrailerFrame` / `serializeChunkedBody`）。`HttpResponse::chunked()` 工厂方法创建 chunked 响应
- `SseSession.h/cpp` — Server-Sent Events 会话封装（RFC 8895），协程化 `sendEvent()` / `sendData()` / `sendComment()` / `close()` 接口，基于 chunked transfer-encoding 流式推送，集成 30 秒心跳保活 + 30 分钟 idle 超时
- `GzipCompression.h/cpp` — Gzip 响应压缩中间件，自动检查 `Accept-Encoding: gzip`，小 body（< 64KB）整体压缩后替换 Content-Length，大 body（>= 64KB）走 chunked 流式压缩。可配置压缩级别（1-9）和最小压缩阈值（默认 1KB）。`makeGzipCompressionMiddleware()` 返回 `SyncAfterHandler`
...
- **Zero-copy HTTP parsing**: picohttpparser parses into stack-allocated `phr_header[64]` array. `NativeRequest` stores `string_view` referencing per-request `readBuf` (borrowed from `ReadBufferPool`, returned after response is written) — zero heap allocation for headers/target. Body uses owned `std::string` (read directly from socket). Single-buffer response serialization: head+body into `FixedBuffer<512>`, one `async_write` call. Idle connections hold no read buffer.
...
- `test_rate_limiter.cpp` — Rate Limiter Token Bucket（11 tests: burst/refill/独立key/429/maxEntries/并发稳定性）
- `test_chunked_sse.cpp` — Chunked Transfer-Encoding + SSE 流式推送
- `test_compression.cpp` — Gzip 压缩中间件（Accept-Encoding 协商、小 body 整体压缩、大 body 流式 chunked）
- `test_wildcard_route.cpp` — 通配路由（匹配优先级、参数提取、方法隔离）
