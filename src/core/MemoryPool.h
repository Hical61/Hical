...
class TrackedResource : public std::pmr::memory_resource
{
  public:
    ...
    size_t totalAllocations() const { return totalAllocations_.load(std::memory_order_relaxed); }
    size_t totalDeallocations() const { return totalDeallocations_.load(std::memory_order_relaxed); }
    size_t currentBytes() const { return currentBytes_.load(std::memory_order_relaxed); }
    size_t peakBytes() const { return peakBytes_.load(std::memory_order_relaxed); }
...
class MemoryPool
{
  public:
    ...
    createRequestPool(size_t initialSize = 0);
     ...
     */
    TrackedResource& trackedResource() { return trackedResource_; }
