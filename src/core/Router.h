...
using WsMessageCallback =
    std::function<Awaitable<void>(const std::string&, WebSocketSession&)>;

/**
 * @brief WebSocket 连接回调类型
 */
using WsConnectCallback =
    std::function<Awaitable<void>(WebSocketSession&)>;
...
class Router
{
  public:
     ...
     */
    void ws(const std::string& path, WsMessageCallback onMessage,
