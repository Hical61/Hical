...
void Router::ws(const std::string& path, WsMessageCallback onMessage,
                WsConnectCallback onConnect)
{
    wsRoutes_.push_back(
        {path, std::move(onMessage), std::move(onConnect)});
}
    ...
    std::unordered_map<std::string, std::string>& params)
{
    ...
    while (!pattern.empty() && !path.empty())
    {
        ...
        pattern = (pSlash == std::string_view::npos)
                      ? std::string_view{}
                      : pattern.substr(pSlash + 1);
        path = (rSlash == std::string_view::npos)
                   ? std::string_view{}
                   : path.substr(rSlash + 1);
