...
TEST(ConceptsTest, AsioBackendTypeAliases)
{
    // 编译通过即验证了类型别名的正确性
    bool isEventLoop =
        std::is_same_v<AsioBackend::EventLoopType, AsioEventLoop>;
    bool isTimer =
        std::is_same_v<AsioBackend::TimerType, AsioTimer>;
    bool isConnection =
        std::is_same_v<AsioBackend::ConnectionType, TcpConnection>;
...
TEST(ConceptsTest, NetworkBackendAsTemplateParam)
{
    ...
    bool isLoop =
        std::is_same_v<BackendTraits<AsioBackend>::Loop, AsioEventLoop>;
    EXPECT_TRUE(isLoop);
}
...
static_assert(!NetworkBackend<IncompleteBackend>,
              "IncompleteBackend must NOT satisfy NetworkBackend concept");

static_assert(!EventLoopLike<int>,
              "int must NOT satisfy EventLoopLike concept");

static_assert(!TcpConnectionLike<int>,
              "int must NOT satisfy TcpConnectionLike concept");

static_assert(!TimerLike<int>,
              "int must NOT satisfy TimerLike concept");
