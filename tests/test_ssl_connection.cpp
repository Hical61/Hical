...
{
    SslContext ctx;
    EXPECT_THROW(ctx.loadCertificate("/nonexistent/cert.pem"),
                 std::runtime_error);
}
...
{
    SslContext ctx;
    EXPECT_THROW(ctx.loadPrivateKey("/nonexistent/key.pem"),
                 std::runtime_error);
}
...
TEST(SslConnectionTest, NativeSslHandshake)
{
    ...
    boost::asio::co_spawn(ioCtx,
        [&]() -> boost::asio::awaitable<void> {
            ...
            auto clientSsl = std::make_shared<
                boost::asio::ssl::stream<tcp::socket>>(
                ioCtx, clientCtx.native());
            ...
            boost::asio::co_spawn(ioCtx,
                [&, port, clientSsl]() -> boost::asio::awaitable<void> {
                    ...
                    co_await clientSsl->async_handshake(
                        boost::asio::ssl::stream_base::client,
                        boost::asio::use_awaitable);
                    // 发送数据
                    std::string msg = "Hello SSL!";
                    co_await boost::asio::async_write(
                        *clientSsl, boost::asio::buffer(msg),
                        boost::asio::use_awaitable);
                    // 等待服务端读完（短暂延时保持连接）
            ...
            // 服务端接受
            tcp::socket rawSocket = co_await acceptor.async_accept(
                boost::asio::use_awaitable);

            boost::asio::ssl::stream<tcp::socket> serverSsl(
                std::move(rawSocket), serverCtx.native());

            // 服务端握手
            co_await serverSsl.async_handshake(
                boost::asio::ssl::stream_base::server,
                boost::asio::use_awaitable);

            // 读取数据
            char buf[1024];
            auto n = co_await serverSsl.async_read_some(
...
TEST(GenericConnectionTest, PlainConnectionAlias)
{
    ...
    static_assert(std::is_same_v<PlainConnection, AsioTcpConnection>,
                  "PlainConnection 应与 AsioTcpConnection 相同");
    SUCCEED();
}
...
{
    static_assert(!PlainConnection::isSsl(),
                  "PlainConnection 不是 SSL 连接");
    static_assert(SslConnection::isSsl(),
