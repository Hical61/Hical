...
createConnectedSockets(boost::asio::io_context& io1,
                       boost::asio::io_context& io2)
{
    using boost::asio::ip::tcp;

    // 创建 acceptor，绑定到 localhost
    tcp::acceptor acceptor(io1, tcp::endpoint(
    ...
    // 异步接受连接
    acceptor.async_accept(serverSocket, [&](boost::system::error_code ec) {
        EXPECT_FALSE(ec);
        connected = true;
    });

    // 客户端连接到 localhost
    clientSocket.connect(tcp::endpoint(
...
{
    AsioEventLoop loop;
    auto [socket1, socket2] = createConnectedSockets(loop.getIoContext(),
                                                      loop.getIoContext());

    InetAddress localAddr("127.0.0.1", 12345);
    InetAddress peerAddr("127.0.0.1", 54321);

    auto conn = std::make_shared<AsioTcpConnection>(
        &loop, std::move(socket1), localAddr, peerAddr);

    EXPECT_FALSE(conn->connected());

    std::atomic<bool> established{false};
    conn->onConnection([&](const TcpConnection::Ptr& c) {
        if (c->connected())
        {
            established = true;
        }
...
TEST(AsioTcpConnectionTest, SendAndReceive)
{
    ...
    conn->onMessage([&](const TcpConnection::Ptr& /*c*/, PmrBuffer* buf) {
...
TEST(AsioTcpConnectionTest, WriteCompleteCallback)
{
    ...
    conn->onWriteComplete([&](const TcpConnection::Ptr&) {
        writeCompleteCount++;
    });
...
TEST(AsioTcpConnectionTest, CloseCallback)
{
    ...
    conn->onClose([&](const TcpConnection::Ptr&) {
        closed = true;
    });
...
TEST(AsioTcpConnectionTest, PmrBufferUsage)
{
    ...
    EXPECT_EQ(std::string(receivedBuffer.peek(), receivedBuffer.readableBytes()),
              testData);
