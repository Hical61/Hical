...
class IntegrationTest : public ::testing::Test
{
  protected:
    ...
    Response sendRequest(http::verb method, const std::string& target,
     ...
     */
    std::vector<Response> sendKeepAliveRequests(
        const std::vector<std::pair<http::verb, std::string>>& requests)
    {
        boost::asio::io_context io;
        tcp::socket sock(io);
        sock.connect(tcp::endpoint(
...
TEST_F(IntegrationTest, ConcurrentRouterDispatch)
{
    ...
    {
        threads.emplace_back([&router, &successCount, t]() {
            boost::asio::io_context io;
...
TEST_F(IntegrationTest, ConcurrentMiddleware)
{
    ...
    pipeline.use(
        [&callCount](const HttpRequest& req, MiddlewareNext next)
