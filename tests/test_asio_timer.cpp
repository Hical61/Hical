...
TEST(AsioTimerTest, RunOnce)
{
    ...
    auto timer = std::make_shared<AsioTimer>(&loop, 0.1, [&]() {
        executed = true;
    });
...
TEST(AsioTimerTest, CancelOnce)
{
    ...
    auto timer = std::make_shared<AsioTimer>(&loop, 0.2, [&]() {
        executed = true;
    });
...
{
    AsioEventLoop loop;
    auto timer = std::make_shared<AsioTimer>(&loop, 1.0, []() {});
