...
int main()
{
    ...
    size_t threads = threadEnv
                         ? static_cast<size_t>(std::atoi(threadEnv))
    ...
    server.router().post("/baseline11",
        [](const HttpRequest& req) -> HttpResponse
        {
            auto aOpt = req.queryParam("a");
            auto bOpt = req.queryParam("b");
            int64_t sum = std::stoll(aOpt.value_or("0"))
                        + std::stoll(bOpt.value_or("0"))
    ...
    server.router().get("/json/{count}",
        ...
        {
            int count = 0;
            try { count = std::stoi(req.param("count")); } catch (...) {}

            int64_t m = 1;
            if (auto mOpt = req.queryParam("m"))
            {
                try { m = std::stoll(*mOpt); } catch (...) {}
            }
